...
const parseSmtpResponse = async (socket, type, successCodes) => {
  let chunksBuffer = Buffer.from('');
  const pass = new PassThrough();
  socket.pipe(pass);
  for await (const chunk of pass) {
    chunksBuffer = Buffer.concat([chunksBuffer, chunk]);
    if (!bufferEndWith(chunksBuffer, SMTP_LINE_END)) continue;
    const separator = readLastSeparator(chunksBuffer);
    if (separator === MULTILINE_RESPONSE_SEPARATOR) continue;
    const { code, data } = parseResponseData(chunksBuffer.toString());
